# HUMAN NOT FOUND — Ingeniería de Software II 

## Integrantes

- Selvin Estuardo Alvarz Juárez — Líder
- William Salvador Ramirez Lacán — Documentación
- Byron Fredy Emmanuel Saquic Ixcol — Documentación
- Yaxchel Saqkan Xol Par — Herramientas
- Hab´il Anayte´ Xol Par — Comunicación
- Hefer Andrés Cotuc Sunú — Herramientas

---

## Proyecto

**404: SURVIVAL **

### Descripción

404: SURVIVAL es un videojuego de terror psicológico donde el jugador debe escapar de un laboratorio mientras sigue las instrucciones de una inteligencia artificial. Al final descubre que toda la experiencia fue una simulación creada para analizar su comportamiento y reemplazar su identidad.

**Docente:** Omar Arreaga

**Ciclo:** 2 2026

**UVG Campus Altiplano**

---

# Herramientas del equipo

- Repositorio: GitHub
- Gestión: Notion
- Diseño UI: Figma
- IA: ChatGPT / Claude / GitHub Copilot

---

# Estructura del repositorio

```
/
├── README.md
├── semana-01/
│   ├── concept-document.md
│   ├── acuerdo-de-equipo.md
│   ├── diagnostico-[nombre].md
│   └── ...
├── documentos/
├── diagramas/
├── prototipos/
└── sprints/
```

### Descripción de las carpetas

**semana-01/**
Contiene todos los entregables correspondientes a la primera semana del proyecto.

**documentos/**
Almacenará la Especificación de Requerimientos de Software (ERS), arquitectura, planificación y demás documentación técnica.

**diagramas/**
Contendrá diagramas UML, diagramas de arquitectura y otros modelos del sistema.

**prototipos/**
Guardará wireframes, diseños de Figma, mockups y recursos visuales.

**sprints/**
Contendrá los resúmenes y avances de cada sprint desarrollado durante el semestre.